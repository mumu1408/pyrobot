...
cd $PROJECT_FOLDER/src/pyrobot/robots/LoCoBot/locobot_description/urdf
ln cmu_locobot_description.urdf locobot_description.urdf
cd $PROJECT_FOLDER/src/pyrobot/robots/LoCoBot/locobot_moveit_config/config
ln cmu_locobot.srdf locobot.srdf
cd $PROJECT_FOLDER/src/pyrobot/robots/LoCoBot/locobot_control/src
...
if [ ! -d "$PROJECT_FOLDER/src/pyrobot/robots/LoCoBot/thirdparty" ]; then
...
cd $PROJECT_FOLDER/src/pyrobot/robots/LoCoBot/install
chmod +x install_orb_slam2.sh
source install_orb_slam2.sh

cd $PROJECT_FOLDER
if [ -d "$PROJECT_FOLDER/devel" ]; then
	rm -rf $PROJECT_FOLDER/devel
fi
if [ -d "$PROJECT_FOLDER/build" ]; then
	rm -rf $PROJECT_FOLDER/build
fi

if [ ! -d "$PROJECT_FOLDER/src/turtlebot" ]; then
	cd $PROJECT_FOLDER/src/
	mkdir turtlebot
	cd turtlebot
...
if [ $PYTHON_VERSION == "2" ]; then
	...
	catkin_make
	echo "source $PROJECT_FOLDER/devel/setup.bash" >> ~/.bashrc
	source $PROJECT_FOLDER/devel/setup.bash
	deactivate
fi
if [ $PYTHON_VERSION == "3" ]; then
	...
	cd $PROJECT_FOLDER/src/pyrobot/robots/LoCoBot
	source ~/${virtualenv_name}/bin/activate
	pip3 install --ignore-installed -r requirements_python3_tb2.txt
	#torch==1.1.0+cpu
	#torchvision==0.3.0+cpu
	deactivate
fi
